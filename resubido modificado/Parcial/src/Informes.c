...
/*
 * brief complemento de la funcion baja, recibe dos listas, elimina una plantilla de la primera, y todas
 * las plantillas relacionadas de la segunda
 * \param *estructura 1
 * \param int len largo 1
 * \param *estructura 2
 * \param int len2 largo 2
 *  \param int id posicion a dar de baja de la primera estructura
 *  \return int -1 si los punteros son NULL, -2 si el ID no existe, 0 si resolvio
 * */
...
/** \brief baja logica a traves de bandera isEmpty buscando por ID
 * \*estructura
 * \param len y len 2 largo estructuras
 * \param int largo
 * \return int(-1) por parametros NULL o id no existe, (-2) si no existen datos para imprimir (0) si resolvio (1) si el usuario ingresa mal los datos
 */
...
/*brief recibe dos estructuras, a traves de la posicion de una, imprime datos de la otra
 * param *estructuras 1 y 2
 * param int posicion, posicion de la primera a relacionar con la segunda
 * param int LARGO el largo de la segunda para comparar el dato en comun
 */
...
/*brief recibe dos listas, y sus largos, para cargar una nueva plantilla de la lista uno
 * relacionandola con un dato de la lista 2
 * \param * listas 1 y 2
 * \param LARGO1 Y LARGO2 largos de listas
 * \return (-1) punteros NULL, (-2) lista llena, (-3) lista vacia, (0) resolvio
 */
...
//1
/*brief recibe dos listas, y cuenta la cantidad de plantillas relacionadas de una en otra lista e imprime
 * los datos de la relacion
 * \param *listas 1 y 2
 * \param LEN1 y LEN2 largos de lista
 * */
...
//2
/*brief cuenta las plantillas que superan una cierta cantidad de un dato especifico, e imprime los datos relacionados
 * \param * listas
 * \param int LEN1 y LEN2 largo de listas
 * \param int CANTJUG, cantidad a superar de un valor especifico de una de las listas
 *
 * */
...
//3
/*brief se ingresa un ID, se busca su posion, y se imprimen los datos de la lista propia y los relacionados con una segunda lista
 * \param * listas 1 y 2
 * \param LEN1 y LEN2 largos de listas
 */
...
//4
/*brief recibe dos listas, e imprime los datos de una, con el dato relacion entre las dos listas
 * \param * listas 1 y 2
 * \param LEN1 y LEN2 largos de listas
 */
...
//6
/*brief imprime la recaudacion maxima por salon, buscando la cantidad de maquinas y la maxima capacidad de monedas
 * de cada maquina
 * \param * listas 1 y 2
 * \param LEN1 y LEN2 largos de listas
 */
...
//7
/*brief compara un dato ingresado por el usuario con un dato ingresado con anterioridad en una variable existente
 * \param * listas 1 y 2
 * \param LEN1 y LEN2 largos de listas
 */
...
//5
/*brief cuenta las plantillas con el dato relacion, e imprime la cantidad
 * \param * listas 1 y 2
 * \param LEN1 y LEN2 largos de listas
 */
void imprimirSalonconMasArcades(Arcade* struc,int LEN1,Salon* struc2, int LEN2)
{
	...
	if(struc!=NULL && LEN1>0 && struc2!=NULL && LEN2>0)
	{
		...
		if(contMax>0)
		{
			...
			printf("\nLa direccion es: %s \n",struc2[iMax].direc);
			printf("\nEl tipo de salon es (1.Local/2.Shopping): %d \n",struc2[iMax].tipo);
		}
	}
}
//8
/*brief imprime los salones que estan igual o por ensima de un requisito
 * \param * listas 1 y 2
 * \param LEN1 y LEN2 largos de listas
 */
void equipadosCompletos(Arcade* struc1, int LEN1,Salon* struc2, int LEN2)
{

	int i;
	int j;
	int contarArcades=0;
	if(struc1!=NULL && LEN1>0 && struc2!=NULL && LEN2>0)
	{
		for(i=0;i<LEN2;i++)//SALON
		{
			contarArcades=0;
			if(struc2[i].isEmpty==OCUPADO)
			{
				for(j=0;j<LEN1;j++)//ARCADES
				{
					if(struc1[j].isEmpty==OCUPADO && struc1[j].idSalon==struc2[i].idSalon)
					{
						if(struc1[j].players>2)
						{
							contarArcades++;
						}
				...
				if(contarArcades>7)
				{
					printf("El salon ID %d NOMBRE %s esta equipado por completo",struc2[i].idSalon,struc2[i].name);
				}
			}
		}
	}
...
/*brief suma la cantidad de plantillas activas de una estructura, y la cantidad de otra e imprime un promedio
 * entre ambas
 * \param * listas 1 y 2
 * \param LEN1 y LEN2 largos de listas
 */
void promedioDeArcades(Arcade* struc1, int LEN1,Salon* struc2, int LEN2)
{
	int i;
	...
	if(struc1!=NULL && LEN1>0 && struc2!=NULL && LEN2>0)
	{
		...
		promedio=contarArcades/contarSalones;
		printf("El promedio de Arcades x Salon es %d",promedio);
	}
}
